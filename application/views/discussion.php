	...
	<div class="large-8 medium-12 small-12 columns">
		<div class="row collapse panel radius post" style="margin-bottom: 15px;">
			...
			<div class="large-9 medium-9 small-6 columns">
				<h3 style="margin-bottom:0"><?= $post['post_title']; ?></h3>
				<? if ($post['edited'] == 1) : ?><b><small>(Edited)</small></b>
				<? endif; ?><br>
				<small>Posted by <b>
						<? if ($login->isUserLoggedIn() == true) : ?><a href="<? if ($post['author_id'] == $_SESSION['user_id']) : ?>/<? else : ?>/users/view/<?= $post['author_id'] ?>/<? endif; ?>"><?= $post['post_author'] ?></a>
						<? else : ?><?= $post['post_author'] ?>
						<? endif ; ?></b> <?= $timeago = $create->timeAgo(strtotime($post['date'])); ?> in</small>
		...
		<div class="row collapse" style="margin-bottom: 15px;">
			<div class="large-12 small-12 columns">
				...
				<div class="tiny radius button right" data-reveal-id="new_reply">Reply To This Discussion</div>
				<? endif; ?>
			</div>
		</div>
		<? if ($replies) : ?>
		...
		<div class="row collapse panel radius post" style="margin-bottom: 15px;">
			...
			<div class="large-10 small-10 columns">
				...
				<small>Posted by <b>
						<? if ($login->isUserLoggedIn() == true && $reply['author_id'] != 0) : ?><a href="<? if ($reply['author_id'] == $_SESSION['user_id']) : ?>/<? else : ?>/users/view/<?= $reply['author_id'] ?>/<? endif; ?>"><?= $reply['reply_author'] ?></a>
						<? else : ?><?= $reply['reply_author'] ?>
						<? endif ; ?></b> <?= $timeago = $create->timeAgo(strtotime($reply['date'])); ?> in</small>
			</div>
			<? if ($login->isUserLoggedIn() == true) : ?>
		...
		<? else : ?>
		<small>No replies yet, here.
			<? if ($login->isUserLoggedIn() == true) : ?>Reply to this message!
			<? else : ?>Log in to reply to this message.
			<? endif; ?></small>
		<? endif; ?>
