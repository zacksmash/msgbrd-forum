	...
	<div class="large-8 medium-12 small-12 columns">
		<div class="row collapse panel radius post" style="margin-bottom: 15px;">
			<div class="large-2 small-2 columns">
				<div style="width:50px; height:50px; background: url('<? if ($avatar = $login->hasAvatar($post['author_id']) == 1) : ?>/public/avatars/avatar-<?= $post['author_id'] ?>.jpg<? else : ?>/public/avatars/default-avatar.png<? endif; ?>') no-repeat center; background-size: 50px; border-radius: 3px;"></div>
			</div>
			<div class="large-10 small-12 columns">
				<span><?= $post['post_title']; ?> <? if ($post['edited'] == 1) : ?><b><small>(Edited)</small></b><? endif; ?></span><br>
		...
		<? if ($replies) : ?>
			...
			<div class="row collapse panel radius post" style="margin-bottom: 15px;">
				<div class="large-2 small-2 columns">
					<div style="width:50px; height:50px; background: url('<? if ($avatar == 1) : ?>/public/avatars/avatar-<?= $reply['author_id'] ?>.jpg<? else : ?>/public/avatars/default-avatar.png<? endif; ?>') no-repeat center; background-size: 50px; border-radius: 3px;"></div>
				</div>
