	...
	<div class="large-8 small-12 columns">
		<div class="row collapse panel radius">
			<div class="large-3 medium-3 small-6 columns" style="padding-right: 15px;">
				<div style="margin-bottom: 15px; width:134px; height:134px; background: url('<? if ($avatar = $login->hasAvatar($profile['user_id']) == 1) : ?>/public/avatars/avatar-<?= $profile['user_id'] ?>.jpg<? else : ?>/public/avatars/default-avatar.png<? endif; ?>') no-repeat center; background-size: 134px; border: 2px solid #efefef; border-radius: 3px;"></div>
			</div>
			<div class="large-9 medium-9 small-6 right columns">
				<div><?= $profile['user_name'] ?></div>
				<hr>
				<? if ($profile['user_twitter']) : ?><div><a href="<?= $profile['user_twitter'] ?>" target="_blank"><i class="icon-twitter"></i> Twitter</a></div><? endif; ?>
				<? if ($profile['user_facebook']) : ?><div><a href="<?= $profile['user_facebook'] ?>" target="_blank"><i class="icon-facebook"></i> Facebook</a></div><? endif; ?>
				<? if ($profile['user_linkedin']) : ?><div><a href="<?= $profile['user_linkedin'] ?>" target="_blank"><i class="icon-linkedin"></i> Linked In</a></div><? endif; ?>
			</div>
			<div class="large-12 small-12 columns">
				<h6><b>About Me:</b></h6>
				<? if ($profile['user_bio']) : ?><span><?= $profile['user_bio'] ?></span><? else : ?><span>Nothing here.</span><? endif; ?>
			</div>
		</div>
	</div>
