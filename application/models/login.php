...
class Login
{
    ...
    private function deleteAccount($user_id, $user_name)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            // database query, deleting all the info of the selected user
            $query_delete_user = $this->db_connection->prepare('DELETE FROM users WHERE user_id = :user_id AND user_name = :user_name');
